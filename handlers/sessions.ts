...
export async function add(sessionData: string[]): Promise<Player> {
  ...
  let player_database: any = await query(
  ...
  let session: Player = new Player(
    player_database[0].id,
    player_database[0].username
  );
  ...
  session.buffer.writePacket(packetIDs.BANCHO_LOGIN_REPLY, (b) =>
    b.writeInt(session.id)
  );
  //  friends go here
  session.buffer.writePacket(packetIDs.BANCHO_PROTOCOL_NEGOTIATION, (b) =>
    b.writeInt(BANCHO_PROTOCOL)
  );
  session.buffer.writePacket(packetIDs.BANCHO_ANNOUNCE, (b) =>
    b.writeString("Logged in Successfully")
  );
  session.buffer.writePacket(packetIDs.BANCHO_LOGIN_PERMISSIONS, (b) =>
    b.writeInt(session.presence.permission)
  );
  ...
  session.buffer.writePacket(packetIDs.BANCHO_HANDLE_OSU_UPDATE, (b) =>
    b.writeStats(session.stats)
  );
  ...
  await query(
    "INSERT INTO logins (user_id, ip, hwid) VALUES (?, '', '')",
    player_database[0].id
  );
  return session;
}
...
export function remove(id: number, reason: string): void {
  ...
  if (sessions[positionOfSession].isSpectating) {
    stopSpectating(
      new ReadOnlySerializationBuffer(),
      sessions[positionOfSession]
    );
  }
  ...
  if (sessions[positionOfSession].spectators.length > 0) {
    for (let i = 0; i < sessions[positionOfSession].spectators.length; i++) {
      stopSpectating(
        new ReadOnlySerializationBuffer(),
        sessions[positionOfSession].spectators[i]
      );
    }
  }
...
export function broadcastToSession(
  buf: SerializationBuffer | Buffer,
  toId: number
): void {
  for (let i = 0; i < sessions.length; i++) {
