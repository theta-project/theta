...
export default async function stopSpectating(reader: ReadOnlySerializationBuffer, session: Player): Promise<void> {
    if (session.isSpectating) {
        let currentlySpectating: Player | undefined = sessionHandler.find(p => p.id === session.spectatingID);
        if (currentlySpectating != null) {
            let placeInSpectators: number = currentlySpectating.spectators.findIndex(p => p.id === session.id);
